...
class DatabaseService {
  ...
  storeMessagesForChat(String chatRoomId, messageMap){
    chatCollection.doc(chatRoomId).collection("chats").add(messageMap).catchError((e){
      print(e.toString());
    });
  }
  ...
  getMessagesForChat(String chatRoomId) async {
    return chatCollection.doc(chatRoomId)
        .collection("chats")
        .orderBy('time')
        .snapshots();
  }
  ...
  getChatLists(String email) async {
    return await chatCollection.where("users", arrayContains: email).snapshots();
  }
